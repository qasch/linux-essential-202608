...
> 
> **Wichtig:** Verlasse nach der Installation die `root` Shell wieder mit dem Kommando `exit`!
...
Erstelle ein neues Verzeichnis mit Namen `metazeichen`. Wechsle hinein mit `cd` und versuche dort eine Datei mit dem Namen `eine datei mit leerzeichen` mit folgendem Kommando zu erstellen:

```
touch eine datei mir leerzeichen
``` 

Untersuche, was passiert und überlege, wie du das Problem umgehen kannst.zu erstellen:

>[!NOTE]
> Suche in der Manpage der `bash` doch einmal nach `QUOTING`.
...
1. Erstelle ein Verzeichnis `foo` mit den beiden Dateien (`regularfile1` und `.hiddenfile` und `.another-hiddenfile`). Erstelle ein Verzeichnis `bar` und verschiebe die Dateien aus `foo` mit dem Kommando `mv ~/foo/* ~/bar` nach `bar`. Prüfe, ob alle Dateien verschoben wurden.

2. Wie kannst du die übriggebliebenen Dateien doch noch verschieben?

3. Geht das sogar in einem Rutsch, also alle Dateien auf einmal zu verschieben?
