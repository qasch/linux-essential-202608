...
> Schaut dazu parallel in die Manpage von `groupadd` um zu verstehen, was die verschiedenen Optionen bedeuten.
>
...
Weist `karla` nun der Gruppe `adm` zu, wieder mit `usermod -G adm karla`. Diese Gruppe existiert bereits. Prüft die Gruppenzugehörigkkeiten von `karla` mit `groups` oder `id`.
...
Erstellt eine Datei mit Namen `karlos-file` im Verzeichnis `/opt` als `root`. Sorgt nun mit dem Kommando `chown karlos:karlos /opt/karlos-file` dafür, dass diese Datei `karlos` gehört bzw. zugewiesen wird (`chown` besprechen wir noch).
