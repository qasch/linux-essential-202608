...
Hole den Prozess nun mit `fg` in den Vordergrund und brich ihn ab. Verwenden dazu die Tastenkombination `STRG C`. Diese Tastenkombination ist in Terminals gleichbedeutend mit *Abbruch* bzw. *Cancel*. Mehr dazu in späteren Übungen.
...
- Bringe nun `sleep` wieder in den Vordergrund mit `fg`. 
- Schicke den `sleep` Prozess nun erneut in den Hintergrund und starte einen weiteren Prozess im Hintergrund mit `sleep 400 &. Nun befinden sich zwei Prozesse im Hintergrund.
- Welcher wird wieder in den Vordergrund geholt wenn du `fg` eingibst?
- Gibt es auch eine Möglichkeit, einen bestimmten Prozess auszuwählen und wieder in den Vordergrund zu holen?
